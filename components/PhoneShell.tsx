...
const LimitedEncounterApp = lazyApp(() => import('../apps/LimitedEncounterApp'));
...
const APP_PRELOAD_ORDER: PreloadableLazy[] = [
  Chat, Character, GroupChat, MomentsApp, MihuiApp, LimitedEncounterApp, EatApp, FarawayApp, Settings, UserApp, CallApp, DateApp, Gallery, WorldbookApp, MemoryPalaceApp, HandbookApp, FAQApp, BrowserApp, VoiceDesignerApp, Appearance, ThemeMaker, QQBridge,
];
...
const APP_BY_ID: Partial<Record<AppID, PreloadableLazy>> = {
  [AppID.Settings]: Settings, [AppID.User]: UserApp, [AppID.Character]: Character, [AppID.Chat]: Chat, [AppID.Date]: DateApp, [AppID.Moments]: MomentsApp, [AppID.Mihui]: MihuiApp, [AppID.LimitedEncounter]: LimitedEncounterApp, [AppID.Eat]: EatApp, [AppID.Faraway]: FarawayApp, [AppID.Handbook]: HandbookApp, [AppID.Appearance]: Appearance,
};
  ...
  <div className="fixed inset-0 z-[9999] flex items-center justify-center p-5 animate-fade-in">
    ...
    <div className="relative w-full max-w-sm bg-white/95 backdrop-blur-xl rounded-[2.5rem] shadow-2xl border border-white/30 overflow-hidden animate-slide-up">
      ...
      <div className="pt-7 pb-3 px-6 text-center">
        ...
        <p className="text-[11px] text-slate-400 mt-1">Disclaimer · Morpho</p>
      </div>

      {/* Content */}
      <div className="px-6 pb-4 max-h-[55vh] overflow-y-auto no-scrollbar space-y-3">
        <p className="text-[13px] text-slate-600 leading-relaxed">
          本项目「Morpho」是一个<strong className="text-slate-800">完全开源、免费</strong>的软件，仅供个人学习、研究与技术交流使用。
        </p>
...
const PhoneShell: React.FC = () => {
      ...
      >
        ...
        <div className="absolute top-24 w-full text-center drop-shadow-[0_2px_4px_rgba(0,0,0,0.3)]">
           ...
           {acnhSkin ? (
               ...
               <div className="text-lg tracking-widest opacity-90 mt-2 uppercase text-xs font-bold">Morpho</div>
           )}
        </div>
  ...
  const renderApp = () => {
    switch (activeApp) {
      ...
      case AppID.LimitedEncounter: return <LimitedEncounterApp />;
