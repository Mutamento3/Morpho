...
/**
 * 轮询模式里一次 API 调用所处的「发言位」。三个位置的处境完全不同，
 * 共用一套话术的话第三次必然复读（模型会以为自己还在常规轮次里）。
 *
 * - opening  第一轮首发：正常展开观点
 * - reply    第一轮接话：能看到首发成员的新消息，正常展开观点
 * - followup 第二轮首发：读完整上下文后简短补充
 * - closing  第二轮接话：读完整上下文后简短收束，随后等待用户
 */
export type RoundRobinSlot = 'opening' | 'reply' | 'followup' | 'closing';
...
export function buildRoundRobinInstruction(
    ...
    const stance = meetingMode
        ...
        ? slot === 'opening'
            ...
            : slot === 'reply'
            ? `你是本回合**第一轮第二个**发言的人：顺着最新聊天接话，只说最重要的内容。目标 **1-3 行**。`
            : slot === 'followup'
            ? `现在进入**第二轮短回应**：只补一个新观点或回应最新一句，不要复述。目标 **1-3 行**，没什么值得补充时允许跳过。`
            : `你是本回合**第二轮最后一个**发言的人：用最短的话回应或收束，然后等待用户。目标 **1-3 行**，没什么值得补充时允许跳过。`
        : slot === 'opening'
...
只补充新观点、回答对方或推进讨论，不要复述第一轮。目标 **1-3 行**，没什么值得补充时允许跳过。`
        : `你是本回合**第二轮最后一个**发言的人：记录末尾已经包含用户原话和前面三次发言。
简短回应最新观点或补上最后一个有价值的信息，然后停下来等待用户参与。不要复述、不要另开大话题，也不要用问题强行延长 AI 之间的对聊。目标 **1-3 行**，没什么值得补充时允许跳过。`;

    // 第一轮保证两位都参与；第二轮仍会实际调用，但允许角色判断无话可补时静默跳过
    const rule1 = isSecondPass
        ? `你只是群里的一位普通成员，不是导演。只输出**你自己**要发的消息内容——不要替任何人说话，不要在开头加自己的名字或冒号前缀，不要解释、不要输出 JSON。
   第二轮如果确实没有新内容可补，**允许只输出 \`[[SKIP]]\`**。别为了凑满四段而重复第一轮或硬造废话。`
        : `你只是群里的一位普通成员，不是导演。只输出**你自己**要发的消息内容——不要替任何人说话，不要在开头加自己的名字或冒号前缀，不要解释、不要输出 JSON。本轮每位成员都会依次发言，**轮到你就要说点什么**——哪怕只是一句"嗯"、接个梗或甩一个表情包。`;
